...
# Configuração primária obrigatória do Streamlit em escopo isolado
...
def main():
    ...
    renderizar_sidebar()
    concessionarias_locais = obter_concessionarias()
...
res = dimensionar_circuito_nbr5410_mda(c_pot_in, dados_c["fase"], c_com_in, c_desc_in)st.session_state.lista_circuitos_calc.append({"CIRC": c_num_in, "DESCRIÇÃO": c_desc_in.upper(), "COMODO": c_amb_in if c_amb_in else "Geral","POT_W": int(c_pot_in), "POT_VA": res["VA"], "FP": res["FP"], "TIPO": "Monofásico","DISJ": f"{res['DISJUNTORES']}A", "CURVA": res["CURVA"], "COND": f"{res['BITOLA']} mm²","FASE": "R", "TENSÃO": int(dados_c["fase"]), "IB": res["IB"], "IB_CORR": res["IB_CORR"],"COMP": c_com_in, "DV": res["DV"]})salvar_dados_permanentes("circuitos", st.session_state.lista_circuitos_calc)st.success(f"Circuito {c_num_in} adicionado!")st.rerun()if st.session_state.lista_circuitos_calc:st.markdown("#### ❌ Remover Circuito Selecionado")col_rem_1, col_rem_2 = st.columns(2)with col_rem_1:opcoes_circ = [circ["CIRC"] for circ in st.session_state.lista_circuitos_calc]circ_para_remover = st.selectbox("Identificação para Excluir:", opcoes_circ)if st.button("🗑️ Remover Circuito"):st.session_state.lista_circuitos_calc = [c for c in st.session_state.lista_circuitos_calc if c["CIRC"] != circ_para_remover]salvar_dados_permanentes("circuitos", st.session_state.lista_circuitos_calc)st.success("Circuito removido!")st.rerun()if st.session_state.lista_circuitos_calc:st.markdown("#### 📋 Prancha MDA - Circuitos Ativos")st.dataframe(pd.DataFrame(st.session_state.lista_circuitos_calc), use_container_width=True, hide_index=True)else:st.info("Nenhum circuito programado ou calculado até o momento.")with tab_hidraulica:if st.session_state.lista_materials_hidraulicos:st.dataframe(pd.DataFrame(st.session_state.lista_materials_hidraulicos), use_container_width=True, hide_index=True)with tab_gas:if st.session_state.lista_materials_gas:st.dataframe(pd.DataFrame(st.session_state.lista_materials_gas), use_container_width=True, hide_index=True)with tab_dados:if st.session_state.lista_materials_dados:st.dataframe(pd.DataFrame(st.session_state.lista_materials_dados), use_container_width=True, hide_index=True)with tab_seguranca:if st.session_state.lista_materials_seguranca:st.dataframe(pd.DataFrame(st.session_state.lista_materials_seguranca), use_container_width=True, hide_index=True)with tab_solar:if st.session_state.lista_materials_solar:st.dataframe(pd.DataFrame(st.session_state.lista_materials_solar), use_container_width=True, hide_index=True)with tab_catalogo:cat_df = listar_materiais_catalogo()if cat_df:st.dataframe(pd.DataFrame(cat_df, columns=["ID", "Segmento", "Etapa", "Material", "Fator Base", "Unidade"]), use_container_width=True, hide_index=True)with tab_pdf:st.write("### 🖨️ Central de Emissão")if st.button("🔄 Preparar Relatório Técnico"):pdf_dados = gerar_pdf_completo_obra()st.download_button(label="📥 Baixar Memorial Técnico Unificado (PDF)",data=pdf_dados,file_name="memorial_de_engenharia_unificado.pdf",mime="application/pdf",key="btn_pdf_real")if name == "main":main()
<FollowUp>
Execute `streamlit run app.py` no terminal e me confirme:
* Se a prancha limpou com sucesso ao mudar de modo.
* Se os **diagramas vetoriais** indexaram os múltiplos circuitos customizados de forma limpa.

Quer adicionar um **fator de faturamento automatizado** para gerar o orçamento total dos insumos com margem de lucro no PDF?
</FollowUp>
