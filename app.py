...
with aba2:
    ...
    for _, linha in TABELA_CABOS.iterrows():
...
with aba3:
    ...
    with c1:
        ...
        st.dataframe(TABELA_CABOS, use_container_width=True)
    with c2:
        st.subheader("Fatores de Agrupamento de Circuitos")
        st.dataframe(TABELA_AGRUPAMENTO, use_container_width=True)
        
    st.subheader("Dicionário de Rendimento e Insumos da Construção Civil")
    st.dataframe(TABELA_SERVICOS_CIVIL, use_container_width=True)
...
with aba4:
    ...
    if todos_itens:
        ...
        st.dataframe(df_final[["Módulo", "Serviço/Item", "Quantidade", "Unidade", "Preço Unitário (R$)", "Total (R$)", "Cabo (mm²)", "Corrente (A)"]], use_container_width=True)
